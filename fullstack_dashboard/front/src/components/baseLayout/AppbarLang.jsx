  ...
  useEffect(() => {
    const fetchCountryData = async () => {
      try {
        ...
        if(defaultCountry){
          let langKey = Object.keys(defaultCountry?.languages)[0]; // ?: 옵셔널 체이닝, 값이 없으면 undefined 반환
          setSelectedCountry({
              ...
              countries.map((country) => {
                if(country?.languages && Object.keys(country?.languages)) {
                  ...
                  return (
                    ...
                    className='drop-item flex items-center gap-x-3 cursor-pointer dark:text-white py-1 px-0 delay-100 ease-in-out transition hover:dark:bg-gray-700 hover:bg-slate-100' 
